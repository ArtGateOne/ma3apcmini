//ma3apcmini v1.0.5 by ArtGateOne
...
color_executor_off = 5;
color_executor_on = 1;
color_logo = 4;
brightness = 0;     //brightness 0-6
executor_fx = 0;    //fx 7-15
...
//MIDI
//display info
console.log("Akai APCmini MA3 OSC");
console.log(" ");
...
//3 logo
output.send('noteon', { note: 27, velocity: color_logo, channel: 0 });
output.send('noteon', { note: 28, velocity: color_logo, channel: 0 });
output.send('noteon', { note: 36, velocity: color_logo, channel: 0 });
output.send('noteon', { note: 43, velocity: color_logo, channel: 0 });
output.send('noteon', { note: 44, velocity: color_logo, channel: 0 });
output.send('noteon', { note: 52, velocity: color_logo, channel: 0 });
output.send('noteon', { note: 59, velocity: color_logo, channel: 0 });
output.send('noteon', { note: 60, velocity: color_logo, channel: 0 });
...
async function sendNotes() {
  for (let x = 0; x < 16; x++) {
    output.send('noteon', { note: x, velocity: 2, channel: 0 });
...
input.on('noteon', function (msg) {//recive midi keys and send to osc
  ...
  else if (msg.note >= 64 && msg.note <= 71) {
    udpPort.send({ address: "/Fader" + (msg.note + 137), args: [{ type: "i", value: 127 }] }, remoteip, remoteport);
...
input.on('noteoff', function (msg) {//recive midi keys and send to osc
  ...
  if (msg.note >= 16 && msg.note <= 23) {
    ...
    udpPort.send({ address: "/Fader" + (msg.note + 137), args: [{ type: "i", value: Fader[msg.note - 64] }] }, remoteip, remoteport);
    output.send('noteon', { note: msg.note, velocity: msg.velocity, channel: 0 });
  }
...
function light_page_button(note) {
  channel = brightness;
  output.send('noteon', { note: page + 81, velocity: 0, channel: 0 });
...
function change_page(page_new) {
  output.send('noteon', { note: page + 81, velocity: 0, channel: 0 });
  output.send('noteon', { note: page_new + 81, velocity: 1, channel: 0 });
  page = page_new;
}
