//ma3apcminimk2color wing1 v1.0.5 by ArtGateOne
...
udpPort.on("message", function (oscMsg, timeTag, info) {
  ...
  */
  else if (oscMsg.address == "/Key201") { light_executor(0, oscMsg.args[0].value, oscMsg.args[1].value); }
  else if (oscMsg.address == "/Key202") { light_executor(1, oscMsg.args[0].value, oscMsg.args[1].value); }
  else if (oscMsg.address == "/Key203") { light_executor(2, oscMsg.args[0].value, oscMsg.args[1].value); }
  else if (oscMsg.address == "/Key204") { light_executor(3, oscMsg.args[0].value, oscMsg.args[1].value); }
  else if (oscMsg.address == "/Key205") { light_executor(4, oscMsg.args[0].value, oscMsg.args[1].value); }
  else if (oscMsg.address == "/Key206") { light_executor(5, oscMsg.args[0].value, oscMsg.args[1].value); }
  else if (oscMsg.address == "/Key207") { light_executor(6, oscMsg.args[0].value, oscMsg.args[1].value); }
  else if (oscMsg.address == "/Key208") { light_executor(7, oscMsg.args[0].value, oscMsg.args[1].value); }
...
input.on('noteon', function (msg) {//recive midi keys and send to osc
  ...
  if (msg.note >= 0 && msg.note <= 7) {
    udpPort.send({ address: "/Key" + (msg.note + 201), args: [{ type: "i", value: 1 }] }, remoteip, remoteport);
  }
...
input.on('noteoff', function (msg) {//recive midi keys and send to osc
  ...
  if (msg.note >= 0 && msg.note <= 7) {
    udpPort.send({ address: "/Key" + (msg.note + 201), args: [{ type: "i", value: 0 }] }, remoteip, remoteport);
  }
  else if (msg.note >= 8 && msg.note <= 15) {
    //do nothing
  }
